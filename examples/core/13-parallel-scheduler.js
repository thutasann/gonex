...
const heavyComputation = (id, iterations) => {
  console.log(`Starting computation ${id}...`);
  let result = 0;
  for (let i = 0; i < iterations; i++) {
    result += Math.sqrt(i) * Math.sin(i) * Math.cos(i);
  }
  console.log(`Computation ${id} completed with result: ${result.toFixed(2)}`);
  return { id, result: result.toFixed(2) };
};
...
const results = await goAll(
  [
    () => heavyComputation(1, 1000000),
    () => heavyComputation(2, 1000000),
    () => heavyComputation(3, 1000000),
    () => heavyComputation(4, 1000000),
  ],
  {
    useWorkerThreads: true,
